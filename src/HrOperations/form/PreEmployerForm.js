...
import baseUrl from "../../../src/config.json";
    ...
    function PreEmployerForm({
        ...
        const {
          ...
          defaultValues: {
            Employer_Code: Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Employer_Code
              ? Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Employer_Code
              : 0,
              Employer_Name:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Employer_Name,
              Industry_Flag:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Industry_Flag,
              Contact_Name:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Contact_Name,
              Contact_Title:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Contact_Title,
              Address_Line_1: Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Address_Line_1,
              Address_Line_2:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Address_Line_2,
              Telephone_number:Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Telephone_number,
              Fax_number: Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Fax_number,
          },
        ...
        <div className="form-group formBoxPreEmployer">
          ...
          <div className="d-flex">
                <FormCheckBox
                    ...
                    value={'Y'}
                    defaultChecked={
                        Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Industry_Flag == "Y" ? true : false
                    }
                    showLabel={true}
                    errors={errors}
                    control={control}
                ...
                <FormCheckBox
                    ...
                    value={'N'}
                    defaultChecked={
                        Red_previous_Employee?.dataSingle?.[0]?.res?.data?.[0]?.Industry_Flag == "N" ? true : false
                    }
                    showLabel={true}
                    errors={errors}
                    control={control}
...
function mapStateToProps({ Red_previous_Employee }) {
  ...
  export default connect(mapStateToProps, Previous_Employers_ACTIONS)(PreEmployerForm);
