...
function PromotionWaitingForm() {
    ...
    async function getPromotionProcessData() {
        await fetch(`${config["baseUrl"]}/promotions/GetTranPromotionListWaiting`, {
            method: "GET",
            headers: {
            ...
            if (response.messsage == "unauthorized") {
                ...
                if (response.messsage == "timeout error") { navigate("/"); }
                    ...
                    if (response.success) {
                        setGetConfirmationData(response?.data[0])
                        setGetConfirmationDataFilter(response?.data[0])
                        console.log("Process Confirmation",response?.data[0])
                        setDataLoader(true);
                    } else {
                        setDataLoader(false);
                        setGetConfirmationDataErr(response.message)
        ...
        <section className="confirmationSection">
            <div className="container-fluid p-0">
                <div className="TransactionConfirmation_listHeader">
                    <span className="">
                        Transaction Promotion Process - List
                    </span>
                </div>
                                        ...
                                        {isGetConfirmationData.map((items) => {
                                            return (
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button className="editBtnTable"><Link to={`/Transaction_Promotion?Process=Process&PromotionId=${items?.Emp_code}`} className="text-white">Process</Link></button>
                                                    </td>
                                                </tr>
                                            )
                                        })
...
export default PromotionWaitingForm;