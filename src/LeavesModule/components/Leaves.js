...
const Leaves = () => {
  ...
  async function GetLeaveType() {
    await fetch(
      ...
      if (response.messsage == "unauthorized") {
        ...
        setGetLeaveType(response.data[0]);
        console.log("Select Leave Type",response.data)
      }
    })
      .catch((error) => {
        setGetLeaveTypeErr(error.message);
  ...
  const submitLeaveApplication = async (e) => {
    ...
    //   "Reason": isLeaveReason ? isLeaveReason : ""
    // })
    // console.log("Check When Submit leave application",checkData)
    // return
    await fetch(
            ...
            <div className="row px-2 mt-2 d-flex justify-content-center">
              <div className="col-lg-10">
                ...
                <div className="form-group w-100">
                  ...
                  <input type="text" readOnly value={isHalfDayFlag == true ? isStoreBalancedDays == "NaN" ? isStoreBalancedDays - 0.5 : isStoreBalancedDays - 0.5 : isRemainingdays} className='form-control' />
                </div>
                <div className="form-group w-100">
